...
#   print('The string contains punctiation')
# else:
...
def check_password_strength(testPass):
  if (len(testPass) >=10 and any(char in string.punctuation for char in testPass) and any(char in string.digits for char in testPass) and any(char in string.ascii_letters for char in testPass)):
    return f'{testPass} is a strong password'
  else:
    return f'{testPass} is not a strong password'
